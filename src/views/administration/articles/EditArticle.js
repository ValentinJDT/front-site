...
const EditArticle = (props) => {
  ...
  const handleSaveArticle = async () => {
    try {
      if (!articleRubrique) throw new Error("Il faut une rubrique");

      const data = await RedacAPI.saveArticle(
        idArticle,
        articleRubrique,
        articleTitle,
        articleContent
  ...
  const _rubrics = rubrics.map((rubric) => (
    <option key={rubric.idRubrique} value={rubric.idRubrique}>
      {rubric.titre}
    </option>
  ));
  ...
  return (
    <>
      ...
      <>
        ...
        <div
          className={"mt-5 mx-auto placeholder-glow"}
          style={{ width: "70%" }}
        >
          <div className={"mb-3"}>
            ...
            <select
              ...
              id="list"
              value={articleRubrique}
              onChange={(e) => setArticleRubrique(e.target.value)}
            >
              {_rubrics}
            </select>
          </div>
