...
const CreateArticle = (props) => {
  ...
  const handleArticle = async () => {
    ...
    try {
      ...
      const rubric = rubrics.filter(
        (rubric) => rubric.titre === inputRubric
      )[0];
  ...
  const _rubrics = rubrics.map((rubric) => (
    <option key={rubric.idRubrique} value={rubric.titre}>
      {rubric.description}
    </option>
  ));
        ...
        <div className={"mt-5 mx-auto"} style={{ width: "70%" }}>
          <div className={"mb-3"}>
            ...
            <div className="input-group">
              ...
              <datalist id="datalistOptions">{_rubrics}</datalist>
              ...
              <button
                ...
                title="Besoin d'une nouvelle rubrique ? Créer la rubrique"
              >
                +
              </button>
            </div>
